...
simple_data_iterator::compute_properties(
  ...
  // Only do when the data contains annotations.
  ...
  if (expected_class_labels.empty()) {
    ...
    // Note that the expected_class_labels is empty only in the training stage,
    // so we cxan make sure classes is valid here.
